<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <!-- Hero Section -->
    <section
      class="py-16 px-4 sm:px-6 lg:px-8 bg-linear-to-br from-navy-50 to-teal-50 dark:from-navy-900 dark:to-gray-800"
    >
      <div class="max-w-7xl mx-auto">
        <div class="text-center mb-12">
          <h1
            class="text-4xl lg:text-5xl xl:text-6xl font-bold text-navy-600 dark:text-navy-400 font-heading leading-tight mb-6"
          >
            Tentang Bahari Sentosa Abadi
          </h1>
          <p
            class="text-xl text-gray-700 dark:text-gray-300 leading-relaxed font-body max-w-4xl mx-auto"
          >
            Membangun kepercayaan sebagai distributor garam terdepan di Indonesia dengan komitmen pada kualitas, integritas, dan pelayanan prima
          </p>
        </div>

        <!-- Company Logo & Overview -->
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <!-- Left side - Company Overview -->
          <div class="space-y-6">
            <div class="flex items-center space-x-4 mb-6">
              <img src="/bsa-logo.avif" alt="Bahari Sentosa Abadi Logo" class="h-20 w-20 rounded-full object-cover" />
              <div class="h-16 w-px bg-gray-300 dark:bg-gray-600"></div>
              <div>
                <h2 class="text-3xl font-bold text-navy-600 dark:text-navy-400 font-heading">Bahari Sentosa Abadi</h2>
                <p class="text-lg text-gray-600 dark:text-gray-400 font-body">Established 2019</p>
              </div>
            </div>
            
            <p
              class="text-lg text-gray-700 dark:text-gray-300 leading-relaxed font-body"
            >
              PT. Bahari Sentosa Abadi didirikan pada tahun 2019 sebagai distributor resmi 
              PT. Unichem Candi Indonesia. Kami berkomitmen untuk menyediakan produk garam 
              berkualitas tinggi untuk industri dan konsumsi dengan standar internasional.
            </p>

            <div class="space-y-4">
              <h3 class="text-xl font-bold text-navy-600 dark:text-navy-400 font-heading">
                Legalitas Perusahaan
              </h3>
              <div class="flex flex-wrap gap-3">
                <UBadge color="primary" variant="soft" size="lg">
                  PKP Terdaftar
                </UBadge>
                <UBadge color="primary" variant="soft" size="lg">
                  SIUP
                </UBadge>
                <UBadge color="primary" variant="soft" size="lg">
                  NIB
                </UBadge>
                <UBadge color="primary" variant="soft" size="lg">
                  Distributor Resmi
                </UBadge>
              </div>
            </div>
          </div>

          <!-- Right side - Company Image -->
          <div class="relative">
            <div
              class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-lg border border-gray-200 dark:border-gray-700"
            >
              <img
                src="/bsa-2.avif"
                alt="Bahari Sentosa Abadi Office Building"
                class="w-full h-96 lg:h-[500px] object-cover"
                loading="lazy"
              />
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- Vision & Mission Section -->
    <section class="py-20 px-4 sm:px-6 lg:px-8 bg-white dark:bg-gray-800">
      <div class="max-w-7xl mx-auto">
        <div class="text-center mb-16">
          <h2
            class="text-3xl lg:text-4xl font-bold text-navy-600 dark:text-navy-400 font-heading mb-4"
          >
            Visi & Misi Perusahaan
          </h2>
          <p
            class="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto font-body"
          >
            Fondasi yang kuat untuk mencapai excellence dalam industri distribusi garam
          </p>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <!-- Visi -->
          <div
            class="bg-linear-to-br from-navy-50 to-navy-100 dark:from-navy-900/20 dark:to-navy-800/20 rounded-2xl p-8 border border-navy-200 dark:border-navy-700 hover:shadow-xl transition-all duration-300"
          >
            <div class="flex items-center space-x-4 mb-6">
              <div
                class="w-16 h-16 bg-navy-600 rounded-full flex items-center justify-center"
              >
                <svg
                  class="w-8 h-8 text-white"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
                  />
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"
                  />
                </svg>
              </div>
              <h3
                class="text-2xl font-bold text-navy-700 dark:text-navy-300 font-heading"
              >
                Visi Kami
              </h3>
            </div>
            <p
              class="text-lg text-gray-700 dark:text-gray-300 leading-relaxed font-body"
            >
              Menjadi distributor garam terkemuka dan terpercaya di Indonesia yang 
              memberikan solusi terbaik bagi kebutuhan industri dan konsumsi dengan 
              standar kualitas internasional, inovasi berkelanjutan, dan komitmen 
              terhadap kepuasan pelanggan.
            </p>
          </div>

          <!-- Misi -->
          <div
            class="bg-linear-to-br from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 rounded-2xl p-8 border border-teal-200 dark:border-teal-700 hover:shadow-xl transition-all duration-300"
          >
            <div class="flex items-center space-x-4 mb-6">
              <div
                class="w-16 h-16 bg-teal-600 rounded-full flex items-center justify-center"
              >
                <svg
                  class="w-8 h-8 text-white"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"
                  />
                </svg>
              </div>
              <h3
                class="text-2xl font-bold text-teal-700 dark:text-teal-300 font-heading"
              >
                Misi Kami
              </h3>
            </div>
            <div class="space-y-4">
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-teal-500 rounded-full mt-2 shrink-0"></div>
                <p class="text-gray-700 dark:text-gray-300 font-body">
                  Menyediakan produk garam berkualitas tinggi dengan sertifikasi internasional
                </p>
              </div>
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-teal-500 rounded-full mt-2 shrink-0"></div>
                <p class="text-gray-700 dark:text-gray-300 font-body">
                  Memberikan layanan distribusi yang handal dan tepat waktu ke seluruh Indonesia
                </p>
              </div>
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-teal-500 rounded-full mt-2 shrink-0"></div>
                <p class="text-gray-700 dark:text-gray-300 font-body">
                  Mengembangkan kemitraan jangka panjang yang saling menguntungkan
                </p>
              </div>
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-teal-500 rounded-full mt-2 shrink-0"></div>
                <p class="text-gray-700 dark:text-gray-300 font-body">
                  Berinovasi dalam teknologi dan proses untuk efisiensi optimal
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- Company History & Milestones -->
    <section
      class="py-20 px-4 sm:px-6 lg:px-8 bg-linear-to-br from-navy-50 to-teal-50 dark:from-gray-900 dark:to-gray-800"
    >
      <div class="max-w-7xl mx-auto">
        <div class="text-center mb-16">
          <h2
            class="text-3xl lg:text-4xl font-bold text-navy-700 dark:text-navy-300 font-heading mb-4"
          >
            Perjalanan Perusahaan
          </h2>
          <p
            class="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto font-body"
          >
            Dari awal berdiri hingga menjadi distributor garam terpercaya di Indonesia
          </p>
        </div>

        <!-- Timeline -->
        <div class="relative">
          <!-- Timeline Line -->
          <div class="absolute left-1/2 transform -translate-x-0.5 w-1 h-full bg-linear-to-b from-navy-400 to-teal-400 rounded-full"></div>

          <!-- Timeline Items -->
          <div class="space-y-12">
            <!-- 2019 - Founding -->
            <div class="relative flex items-center">
              <div class="flex-1 pr-8 text-right">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
                  <h3 class="text-xl font-bold text-navy-700 dark:text-navy-300 font-heading mb-2">
                    2019 - Pendirian Perusahaan
                  </h3>
                  <p class="text-gray-700 dark:text-gray-300 font-body">
                    PT. Bahari Sentosa Abadi didirikan sebagai distributor resmi PT. Unichem Candi Indonesia. 
                    Memperoleh semua izin legal termasuk PKP dan SIUP untuk operasional bisnis.
                  </p>
                </div>
              </div>
              
              <!-- Timeline Dot -->
              <div class="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-navy-600 rounded-full border-4 border-white dark:border-gray-900 z-10"></div>
              
              <div class="flex-1 pl-8">
                <div class="w-16 h-16 bg-linear-to-br from-navy-500 to-navy-600 rounded-full flex items-center justify-center">
                  <span class="text-white font-bold text-lg">2019</span>
                </div>
              </div>
            </div>

            <!-- 2020-2021 - Growth -->
            <div class="relative flex items-center">
              <div class="flex-1 pr-8">
                <div class="w-16 h-16 bg-linear-to-br from-teal-500 to-teal-600 rounded-full flex items-center justify-center ml-auto">
                  <span class="text-white font-bold text-sm">20-21</span>
                </div>
              </div>
              
              <!-- Timeline Dot -->
              <div class="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-teal-600 rounded-full border-4 border-white dark:border-gray-900 z-10"></div>
              
              <div class="flex-1 pl-8">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
                  <h3 class="text-xl font-bold text-teal-700 dark:text-teal-300 font-heading mb-2">
                    2020-2021 - Ekspansi Pasar
                  </h3>
                  <p class="text-gray-700 dark:text-gray-300 font-body">
                    Berhasil memperluas jaringan distribusi ke berbagai kota di Jawa, Sumatera, 
                    dan Kalimantan. Mencapai 25+ klien tetap dengan volume distribusi 15.000+ ton.
                  </p>
                </div>
              </div>
            </div>

            <!-- 2022-2023 - Certification & Quality -->
            <div class="relative flex items-center">
              <div class="flex-1 pr-8 text-right">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
                  <h3 class="text-xl font-bold text-navy-700 dark:text-navy-300 font-heading mb-2">
                    2022-2023 - Sertifikasi Kualitas
                  </h3>
                  <p class="text-gray-700 dark:text-gray-300 font-body">
                    Memperoleh sertifikasi Halal MUI, ISO 9001, ISO 14000, dan FSSC ISO 22000. 
                    Meningkatkan sistem manajemen kualitas dan supply chain management.
                  </p>
                </div>
              </div>
              
              <!-- Timeline Dot -->
              <div class="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-navy-600 rounded-full border-4 border-white dark:border-gray-900 z-10"></div>
              
              <div class="flex-1 pl-8">
                <div class="w-16 h-16 bg-linear-to-br from-navy-500 to-navy-600 rounded-full flex items-center justify-center">
                  <span class="text-white font-bold text-sm">22-23</span>
                </div>
              </div>
            </div>

            <!-- 2024 - Present -->
            <div class="relative flex items-center">
              <div class="flex-1 pr-8">
                <div class="w-16 h-16 bg-linear-to-br from-teal-500 to-teal-600 rounded-full flex items-center justify-center ml-auto">
                  <span class="text-white font-bold text-lg">2024</span>
                </div>
              </div>
              
              <!-- Timeline Dot -->
              <div class="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-teal-600 rounded-full border-4 border-white dark:border-gray-900 z-10"></div>
              
              <div class="flex-1 pl-8">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
                  <h3 class="text-xl font-bold text-teal-700 dark:text-teal-300 font-heading mb-2">
                    2024 - Leadership Position
                  </h3>
                  <p class="text-gray-700 dark:text-gray-300 font-body">
                    Mencapai 70+ klien di seluruh Indonesia dengan 50.000+ ton garam terdistribusi. 
                    Mengintegrasikan teknologi digital untuk efisiensi operasional dan customer service 24/7.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>

    <!-- Core Values Section -->
    <section class="py-20 px-4 sm:px-6 lg:px-8 bg-white dark:bg-gray-800">
      <div class="max-w-7xl mx-auto">
        <div class="text-center mb-16">
          <h2
            class="text-3xl lg:text-4xl font-bold text-navy-600 dark:text-navy-400 font-heading mb-4"
          >
            Nilai-Nilai Perusahaan
          </h2>
          <p
            class="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto font-body"
          >
            Fondasi karakter yang membentuk budaya kerja dan komitmen kami terhadap excellence
          </p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <!-- Value 1: Quality -->
          <div
            class="group bg-linear-to-br from-navy-50 to-navy-100 dark:from-navy-900/20 dark:to-navy-800/20 rounded-xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-navy-200 dark:border-navy-700"
          >
            <div
              class="w-16 h-16 bg-navy-600 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
            >
              <svg
                class="w-8 h-8 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
                />
              </svg>
            </div>
            <h3
              class="text-xl font-bold text-navy-700 dark:text-navy-300 font-heading mb-4"
            >
              Kualitas (Quality)
            </h3>
            <p
              class="text-gray-700 dark:text-gray-300 font-body leading-relaxed"
            >
              Berkomitmen memberikan produk dengan standar tertinggi dan proses 
              quality control yang ketat untuk memastikan kepuasan pelanggan.
            </p>
          </div>

          <!-- Value 2: Integrity -->
          <div
            class="group bg-linear-to-br from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 rounded-xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-teal-200 dark:border-teal-700"
          >
            <div
              class="w-16 h-16 bg-teal-600 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
            >
              <svg
                class="w-8 h-8 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"
                />
              </svg>
            </div>
            <h3
              class="text-xl font-bold text-teal-700 dark:text-teal-300 font-heading mb-4"
            >
              Integritas (Integrity)
            </h3>
            <p
              class="text-gray-700 dark:text-gray-300 font-body leading-relaxed"
            >
              Menjunjung tinggi kejujuran, transparansi, dan etika bisnis dalam 
              setiap transaksi serta membangun kepercayaan jangka panjang.
            </p>
          </div>

          <!-- Value 3: Innovation -->
          <div
            class="group bg-linear-to-br from-navy-50 to-navy-100 dark:from-navy-900/20 dark:to-navy-800/20 rounded-xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-navy-200 dark:border-navy-700"
          >
            <div
              class="w-16 h-16 bg-navy-600 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
            >
              <svg
                class="w-8 h-8 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"
                />
              </svg>
            </div>
            <h3
              class="text-xl font-bold text-navy-700 dark:text-navy-300 font-heading mb-4"
            >
              Inovasi (Innovation)
            </h3>
            <p
              class="text-gray-700 dark:text-gray-300 font-body leading-relaxed"
            >
              Terus mengembangkan proses, teknologi, dan layanan untuk memberikan 
              solusi terbaik yang efisien dan berkelanjutan.
            </p>
          </div>

          <!-- Value 4: Customer Focus -->
          <div
            class="group bg-linear-to-br from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 rounded-xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-teal-200 dark:border-teal-700"
          >
            <div
              class="w-16 h-16 bg-teal-600 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
            >
              <svg
                class="w-8 h-8 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
                />
              </svg>
            </div>
            <h3
              class="text-xl font-bold text-teal-700 dark:text-teal-300 font-heading mb-4"
            >
              Fokus Pelanggan
            </h3>
            <p
              class="text-gray-700 dark:text-gray-300 font-body leading-relaxed"
            >
              Mengutamakan kebutuhan dan kepuasan pelanggan dengan layanan personal, 
              respon cepat, dan solusi yang disesuaikan.
            </p>
          </div>

          <!-- Value 5: Reliability -->
          <div
            class="group bg-linear-to-br from-navy-50 to-navy-100 dark:from-navy-900/20 dark:to-navy-800/20 rounded-xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-navy-200 dark:border-navy-700"
          >
            <div
              class="w-16 h-16 bg-navy-600 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
            >
              <svg
                class="w-8 h-8 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
                />
              </svg>
            </div>
            <h3
              class="text-xl font-bold text-navy-700 dark:text-navy-300 font-heading mb-4"
            >
              Keandalan (Reliability)
            </h3>
            <p
              class="text-gray-700 dark:text-gray-300 font-body leading-relaxed"
            >
              Memastikan kontinuitas pasokan dengan komitmen tepat waktu, konsistensi 
              kualitas, dan sistem backup yang solid.
            </p>
          </div>

          <!-- Value 6: Sustainability -->
          <div
            class="group bg-linear-to-br from-teal-50 to-teal-100 dark:from-teal-900/20 dark:to-teal-800/20 rounded-xl p-8 hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-teal-200 dark:border-teal-700"
          >
            <div
              class="w-16 h-16 bg-teal-600 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"
            >
              <svg
                class="w-8 h-8 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"
                />
              </svg>
            </div>
            <h3
              class="text-xl font-bold text-teal-700 dark:text-teal-300 font-heading mb-4"
            >
              Keberlanjutan
            </h3>
            <p
              class="text-gray-700 dark:text-gray-300 font-body leading-relaxed"
            >
              Menjalankan bisnis dengan memperhatikan dampak lingkungan dan 
              kontribusi positif terhadap masyarakat dan ekosistem industri.
            </p>
          </div>
        </div>
      </div>
    </section>


    <!-- Call to Action -->
    <section
      class="py-20 px-4 sm:px-6 lg:px-8 bg-linear-to-r from-navy-600 to-teal-600 dark:from-navy-800 dark:to-teal-800"
    >
      <div class="max-w-4xl mx-auto text-center">
        <h2
          class="text-3xl lg:text-4xl font-bold text-white font-heading mb-6"
        >
          Bergabunglah dengan Mitra Terpercaya
        </h2>
        <p
          class="text-xl text-navy-100 dark:text-navy-200 font-body leading-relaxed mb-8"
        >
          Jadilah bagian dari jaringan distribusi garam terdepan di Indonesia. 
          Mari wujudkan kesuksesan bersama dengan kualitas, integritas, dan inovasi.
        </p>
        
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <UButton
            :to="`https://wa.me/6281330310676?text=Halo%20Bahari%20Sentosa%20Abadi,%20saya%20tertarik%20untuk%20bermitra%20dengan%20perusahaan%20Anda`"
            target="_blank"
            color="neutral"
            size="xl"
            class="px-10 py-5 transform hover:scale-105 transition-all duration-300 shadow-lg"
          >
            <svg
              class="w-6 h-6 mr-2"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.088"
              />
            </svg>
            Hubungi Kami Sekarang
          </UButton>

          <div class="flex items-center space-x-2 text-navy-100 dark:text-navy-200">
            <svg
              class="w-5 h-5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
              />
            </svg>
            <span class="text-sm font-body">Response dalam 1-2 jam</span>
          </div>
        </div>
      </div>
    </section>

  </div>
</template>